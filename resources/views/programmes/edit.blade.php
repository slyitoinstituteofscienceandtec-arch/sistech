...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1">Edit Programme</h4>
        <p class="text-muted mb-0" style="font-size:13px;">Update programme information</p>
    </div>
    ...
    <div class="col-lg-8">
        <form method="POST" action="{{ route('admin.programmes.update', $programme) }}">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-12">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" style="font-size:13px;border-radius:8px;" value="{{ old('name', $programme->name) }}" required>
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" style="font-size:12px;font-weight:600;">Programme Code <span class="text-danger">*</span></label>
                            <input type="text" name="code" class="form-control @error('code') is-invalid @enderror" style="font-size:13px;border-radius:8px;" value="{{ old('code', $programme->code) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="level" class="form-select @error('level') is-invalid @enderror" style="font-size:13px;border-radius:8px;" required>
                                ...
                                <option value="certificate" {{ old('level', $programme->level) == 'certificate' ? 'selected' : '' }}>Certificate</option>
                                <option value="diploma" {{ old('level', $programme->level) == 'diploma' ? 'selected' : '' }}>Diploma</option>
                                <option value="hnd" {{ old('level', $programme->level) == 'hnd' ? 'selected' : '' }}>HND</option>
                                <option value="professional" {{ old('level', $programme->level) == 'professional' ? 'selected' : '' }}>Professional</option>
                                <option value="short_course" {{ old('level', $programme->level) == 'short_course' ? 'selected' : '' }}>Short Course</option>
                            </select>
                            @error('level') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" style="font-size:12px;font-weight:600;">Duration (Months) <span class="text-danger">*</span></label>
                            <input type="number" name="duration_months" class="form-control @error('duration_months') is-invalid @enderror" style="font-size:13px;border-radius:8px;" value="{{ old('duration_months', $programme->duration_months) }}" min="1" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="is_active" class="form-select @error('is_active') is-invalid @enderror" style="font-size:13px;border-radius:8px;">
                                <option value="1" {{ old('is_active', $programme->is_active ? '1' : '0') === '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $programme->is_active ? '1' : '0') === '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('is_active') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-12">
                            <label class="form-label" style="font-size:12px;font-weight:600;">Description</label>
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" style="font-size:13px;border-radius:8px;" rows="4">{{ old('description', $programme->description) }}</textarea>
            ...
            <div class="d-flex justify-content-end gap-2 mb-3">
                ...
                <button type="submit" class="btn btn-sistech"><i class="fas fa-save me-1"></i> Update Programme</button>
            </div>
        </form>
    </div>
</div>
