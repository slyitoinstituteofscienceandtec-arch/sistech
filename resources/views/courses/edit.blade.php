...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted mb-0" style="font-size:13px;">Update course information</p>
    </div>
    ...
    <div class="col-lg-8">
        <form method="POST" action="{{ route('admin.courses.update', $course) }}">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="code" class="form-control @error('code') is-invalid @enderror" style="font-size:13px;border-radius:8px;" value="{{ old('code', $course->code) }}" required>
                            @error('code') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" style="font-size:12px;font-weight:600;">Course Name <span class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" style="font-size:13px;border-radius:8px;" value="{{ old('name', $course->name) }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="credit_units" class="form-control @error('credit_units') is-invalid @enderror" style="font-size:13px;border-radius:8px;" value="{{ old('credit_units', $course->credit_units) }}" min="1" max="10" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="level" class="form-select @error('level') is-invalid @enderror" style="font-size:13px;border-radius:8px;" required>
                                ...
                                @for($i = 100; $i <= 500; $i += 100)
                                    <option value="{{ $i }}" {{ old('level', $course->level) == $i ? 'selected' : '' }}>{{ $i }} Level</option>
                                @endfor
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="lecturer_id" class="form-select @error('lecturer_id') is-invalid @enderror" style="font-size:13px;border-radius:8px;">
                                ...
                                @foreach($lecturers as $lecturer)
                                    <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $course->lecturer_id) == $lecturer->id ? 'selected' : '' }}>{{ $lecturer->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="is_active" class="form-select @error('is_active') is-invalid @enderror" style="font-size:13px;border-radius:8px;">
                                <option value="1" {{ old('is_active', $course->is_active ? '1' : '0') === '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $course->is_active ? '1' : '0') === '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('is_active') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-12">
                            <label class="form-label" style="font-size:12px;font-weight:600;">Description</label>
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" style="font-size:13px;border-radius:8px;" rows="4">{{ old('description', $course->description) }}</textarea>
            ...
            <div class="d-flex justify-content-end gap-2 mb-3">
                ...
                <button type="submit" class="btn btn-sistech"><i class="fas fa-save me-1"></i> Update Course</button>
            </div>
        </form>
    </div>
</div>
